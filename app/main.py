...
def resumenes_por_vereda(eventos):
    ...
    def valores(col):
        if col not in df.columns: return pd.Series("", index=df.index)
    ...
    return df.groupby("codigo_ver_resuelto", sort=False).agg(
        SIGOber_situaciones=("codigo_ver_resuelto", "size"), SIGOber_anios=("_anio", uniq),
        SIGOber_tipos=("_tipo", uniq), SIGOber_confianza=("_conf", uniq)).reset_index()
...
def construir_mapa(topo, eventos_historicos, conflictos=None, codigo_seleccionado="", mostrar_conflictos=True, pbot_seleccionadas=()):
...
st.markdown("<div class='sigo-subtitle'>Gobernabilidad territorial rural en Puerto Rico, Caquetá · situaciones, actores, cartografía y capacidad institucional.</div></div>", unsafe_allow_html=True)
...
if modo_presentacion: st.caption("¿Dónde? La vereda funciona como unidad de lectura territorial. El mapa permite pasar de la escala municipal a una lectura localizada sin perder la trazabilidad de la evidencia.")
else: st.caption("Seleccione una vereda y, si lo necesita, filtre las situaciones documentadas. Las capas PBOT se mantienen opcionales para conservar fluidez.")
...
mapa,segundos_mapa=construir_mapa(topo,eventos_f,conflictos,codigo_sel,mostrar,tuple(pbot_seleccionadas)); st.caption(f"Generación del mapa en servidor: {segundos_mapa:.2f} s"); st_folium(mapa,width="100%",height=650,returned_objects=["last_active_drawing"])

if modo_presentacion:
    st.markdown("<div class='sigo-section'>03 · Actores</div>", unsafe_allow_html=True)
    st.caption("¿Quiénes intervienen? La fuente de actores permite desplazar la lectura desde el lugar donde ocurre una situación hacia los sujetos y organizaciones con presencia territorial.")
    x,y=st.columns(2)
    ...
    st.markdown("<div class='sigo-section'>04 · Capacidad institucional</div>", unsafe_allow_html=True)
    st.caption("¿Con qué capacidad? Los indicadores SADCI permiten complementar la lectura territorial con evidencia sobre recursos, ejecución y desempeño institucional.")
    if sadci_resumen:
        q1,q2,q3=st.columns(3)
        q1.metric("Ejecución presupuestal",indicador_pct(sadci_resumen.get("ejecucion_presupuestal_pct")))
        q2.metric("Cumplimiento PDT",indicador_pct(sadci_resumen.get("cumplimiento_pdt_pct")))
        personal=(sadci_resumen.get("num_personal_planta",0)+sadci_resumen.get("num_personal_contratista",0)) if ("num_personal_planta" in sadci_resumen or "num_personal_contratista" in sadci_resumen) else None
        q3.metric("Personal promedio",f"{personal:.0f}" if personal is not None else "—")
    ...
    st.caption("Cierre sugerido: El objetivo no es construir un mapa más completo. Es construir una mejor capacidad de lectura del territorio para apoyar decisiones de gobernabilidad rural.")
