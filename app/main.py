...
def popup_conflicto(row):
    ...
    return (
        ...
        "<h4>Situación registrada</h4>"
        f"<b>ID:</b> {v('id_conflicto')}<br>"
        f"<b>Tipo:</b> {v('tipo_conflicto')}<br>"
        f"<b>Vereda:</b> {v('vereda')}<br>"
        f"<b>Descripción:</b> {v('descripcion')}<br>"
        f"<b>Registrado por:</b> {v('registrado_por')}<br>"
        f"<b>Estado coordenada:</b> {v('precision_coordenada')}<br>"
        f"<b>Lat/Lon fuente:</b> {v('lat')} / {v('lon')}"
        "</div>"
    )
...
if not eventos_f.empty:
    ...
    ys = f1.multiselect("Año", anios, default=[])
    tipos = sorted([x for x in eventos_f.get("tipo_conflicto", pd.Series(dtype=str)).astype(str).unique() if x])
